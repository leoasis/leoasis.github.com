...
It's also worth mentioning that this also has security benefits, since by only allowing persisted queries to be executed, we don't care about bad users attempting to craft potentially dangerous queries. Only the ones we wrote in the client (and tested and validated before deploying) are the ones that will be executed. There's another blog post (link) where Max Stoiber (link) talks about other alternatives to security in graphql without using persisted queries.
